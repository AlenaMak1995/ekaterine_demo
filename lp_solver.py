...
def solve_shortest_path_lp_gurobi(mdp: GridWorld, verbose: bool = False, env=None):
    ...
    sa_index: Dict[Tuple[State, Action], int] = {
        (s, a): j for j, (s, a) in enumerate(sa_list)
    }
    ...
    # 3) Build sparse A and b for flow constraints:
    #    out(s) - expected inflow(s) = b_s
    ...
    for s in mdp.states:
        ...
        for a in mdp.actions_from(s):
            ...
            # expected inflow to successors under stochastic kernel
